  ...
  loadPyodide = async function() {
    if (!window.loadPyodide) {
      ...
      await new Promise((resolve, reject) => {
        ...
        script.onerror = (err) => {
          reject(new Error(`Failed to load Pyodide script: ${err}`))
        }
      })
    }
...
async function main() {
  // Load the pyodide loader function first
  const pyodideLoader = await loadPyodide()
...
// Initialize pyodide only in browser
if (typeof window !== 'undefined') {
  main().then((result) => {
    pyodide = result
    reset()
  ...
  async function validate(preCode, setOut, validate) {
    ...
    const out = await pyodide
      ...
      .then(() => {
        ...
        // Dedent validation code to handle JSX template literal indentation issues
        ...
        const dedentedValidation = lines
          .map(line => line.slice(minIndent))
          .join('\n')
        let status = pyodide.runPython(dedentedValidation)
        status = status.toJs()
        return status
      })
      .catch((err) => {
  ...
  return (
    <>
      <div className="not-prose relative my-6 overflow-hidden rounded-2xl bg-zinc-900 shadow-md dark:ring-1 dark:ring-white/10">
        ...
        <div className="absolute bottom-2 right-6 z-10 float-right flex items-end gap-2">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="currentColor"
            className="h-5 w-5 cursor-pointer fill-gray-500 hover:fill-gray-300"
            onClick={() => {
          ...
          <svg
            ...
            viewBox="0 0 24 24"
            fill="currentColor"
            className="h-8 w-8 cursor-pointer fill-green-500 hover:fill-green-200"
            onClick={run}
          >
            <path
              fillRule="evenodd"
